...
Para o desenvolvimento deste estudo, utilizaram-se os conjunto de dados detalhados na Tabela~\ref{tab:conjuntos}. O foco principal recai
sobre os dados do \textit{Article Bias Prediction} (ABP) \cite{baly2020we}, composto por $37.554$ notícias em inglês rotuladas em três categorias:
\textit{left, center e right}. A confiabilidade desses rótulos provém da plataforma AllSides\footnote{\url{https://www.allsides.com/media-bias/media-bias-rating-methods}}, que
emprega auditorias rigorosas e revisões da comunidade.%% conforme ilustrado na Figura~\ref{fig:example_article}. 

\begin{table}[ht]
\centering
\caption{Resumo dos conjuntos de dados e volume de amostras.}
\label{tab:conjuntos}
\begin{tabular}{@{}llc@{}}
\toprule
\textbf{Conjunto de Dados} & \textbf{Classes} & \textbf{Quantidade} \\ \midrule
FlipBias \cite{chen2018learning}        & Left/Center/Right          & 3.066                 \\
ABP \cite{baly2020we}                   & Left/Center/Right          & 37.554                 \\ \bottomrule
\end{tabular}
\end{table}
...
A robustez metodológica foi validada por dois critérios de particionamento. O primeiro, \textit{media-bias split} (Tabela~\ref{tab:particoes_media}), 
segrega os artigos de modo que os dados de uma mesma fonte de notícia sejam exclusivos a uma única partição, impedindo que veículos presentes no conjunto de treinamento apareçam na validação ou teste. 
Essa estratégia é fundamental para mitigar o vazamento de dados e avaliar a capacidade de generalização para fontes não vistas. O segundo, \textit{random split} (Tabela~\ref{tab:particoes_random}), 
mantém a consistência na distribuição de classes entre treino e validação por meio de uma amostragem aleatória.
...
Adicionalmente, o experimento foi estendido para um cenário de generalização externa, onde o modelo treinado integralmente no conjunto ABP foi submetido à 
avaliação no conjunto \textit{FlipBias} \cite{chen2018learning}. Este conjunto contém $2.781$ eventos sob múltiplas perspectivas es serve como referência para medir a eficácia das
representações em contextos fora do domínio de treinamento, seguindo as mesmas diretrizes estabelecidas por Lin et al. [2025]. 

\subsection{Tarefa de Geração de \textit{Embeddings}}\label{sec:representacoes}

Para a execução da tarefa, foi utilizado um modelo fundamentado no \textit{Bidirectional Encoder Representations from Transformers (BERT)} \cite{bert}, 
...
A seleção recaiu sobre o DistilRoBERTa \cite{roberta}, variante destilada que preserva a robustez da arquitetura original, 
contudo, apresenta reduções substanciais no custo computacional e nos requisitos de memória.
...
taxa de aprendizado de $10^{-4}$ e \textit{batch size} de $16$. Para mitigar o \textit{overfitting} e assegurar a capacidade de 
generalização dos modelos, aplicou-se a técnica de \textit{Early Stopping} com paciência de $5$ ciclos, monitorando-se a 
...
Nesta abordagem, o codificador (DistilRoBERTa) ajusta os pesos de suas camadas para otimizar a 
...
Após o mapeamento dos \textit{embeddings}, onde a proximidade entre os vetores reflete a similaridade ideológica das notícias.
...
A confiabilidade do experimento foi assegurada pela validação cruzada \textit{k-folds}, utilizando $5$ \textit{folds} . Esse procedimento garante que a proporção das classes
