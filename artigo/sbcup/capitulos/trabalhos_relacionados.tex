...
Para mitigar essa lacuna, o framework \textit{POLITICS} \cite{liu-etal-2022-politics} introduziu o pré-treinamento de modelos RoBERTa via (\textit{triplet loss}), comparando artigos de diferentes espectros sobre a mesma notícia. 
Esse esforço foi aprimorado por Volf and Simko [2025], que realizaram o ajuste fino (\textit{fine-tuning}) do modelo utilizando múltiplos datasets unificados. Os autores demonstraram que essa especialização, aliada a 
uma filtragem prévia de textos políticos, é essencial para assegurar a robustez e a generalização dos classificadores em diversos domínios.
