...
Uma análise comparativa foi realizada com uma amostra de $90$ instâncias do teste \textit{media-bias split} para situar o modelo frente a LLMs de última 
geração. Observa-se que o modelo proposto, mesmo sendo uma arquitetura destilada e leve (DistilRoBERTa), superou o desempenho \textit{zero-shot} do DeepSeek ($16,0\%$) 
e aproximou-se da performance do Gemini 3 ($34,0\%)$. Isso valida a eficácia da \textit{Contrastive Loss} em gerar representações discriminatórias competitivas, 
mesmo sob restrição de dados e poder computacional.
