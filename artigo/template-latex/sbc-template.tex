...
Para testar os limites dessa generalização em um cenário de estresse ainda mais rigoroso, 
a Tabela~\ref{tab:resultados_flibias} apresenta os resultados de transferência de domínio no conjunto 
\textit{FlipBias}. Diferente dos \textit{baselines} de Lin et al. [2024, 2025], que foram treinados e testados 
integralmente no \textit{FlipBias}, as configurações propostas foram treinadas no \textit{dataset} ABP e avaliadas 
sem exposição prévia ao novo domínio. Embora os experimentos $16$ e $4$ tenham obtido \textit{Macro F1-Scores} 
de $47,00\%$ e $28,21\%$ — valores inferiores aos modelos \textit{in-domain}\footnote{In-domain refere-se a uma situação em que os dados utilizados 
para treinar um modelo de aprendizagem de máquina e os dados usados para testá-lo vêm da mesma distribuição, contexto ou fonte.} —, os dados evidenciam os 
desafios intrínsecos da avaliação \textit{cross-domain}\footnote{Cross-domain refere-se ao treinamento de um modelo de aprendizagem de máquina em um determinado contexto e avaliado em um contexto diferente.}. Essa análise valida a arquitetura ao demonstrar 
que, mesmo sem ajuste fino no domínio específico, o modelo captura estruturas ideológicas fundamentais, 
embora a especialização no domínio de destino continue sendo um fator determinante para o desempenho superior.
...
Por outro lado, os experimentos no \textit{FlipBias} revelaram as fronteiras da generalização \textit{cross-domain}, 
...
os modelos para análises multilíngues e testar modelos de \textit{Large Language Models} (\textit{LLMs}).
