...
Os experimentos foram conduzidos seguindo a metodologia proposta, totalizando $24$ configurações que integraram os modelos  
DistilBERT e DistilRoBERTa, funções de perda (\textit{Contrastive Loss} e \textit{Triplet Loss}), e classificadores como 
\textit{KNN}, \textit{K-Means} e \textit{MLP}. Essa abordagem permitiu uma análise sistemática da sensibilidade dos modelos nos cenários \textit{random split} e 
\textit{media-bias split}, cujo os melhores desempenhos estão sintetizados na Tabela~\ref{tab:resultados_ajustados} em 
contraste com os \textit{baselines} da literatura.
...
Os resultados demonstram que o uso da \textit{Contrastive Loss} favorece a classificação de viés ideológico ao otimizar 
a separação vetorial no espaço de \textit{embeddings}. Conforme a Tabela Tabela~\ref{tab:resultados_ajustados}, o modelo 
proposto (\textit{DistilRoBERTa} + \textit{KNN}) atingiu $45,44\%$ de \textit{Macro F1-score} no cenário \textit{media-bias split}, 
superando a abordagem de Baly et al. [2020] em aproximadamente $12$ pontos percentuais. No entanto, a análise dos 
particionamentos revela que o desempenho é significativamente superior no \textit{random split} ($83,90\%$), onde 
a presença de fontes comuns entre treino e teste facilita a predição. Já o \textit{media-bias split} impõe 
um desafio de generalização maior, exigindo que o modelo identifique nuances ideológicas em portais inéditos, 
servindo como um estágio intermediário entre a memorização de fontes e a generalização plena.
...
\begin{tabular}{@{}cccc@{}}
\toprule
\textbf{Modelo}                                              & \textbf{\begin{tabular}[c]{@{}c@{}}Configuração\\ (Repre. / Fun. de Perda / Classificador)\end{tabular}} & \textbf{Estratégia} & \textbf{Macro F1-Score} \\ \midrule
...
\begin{table}[!ht]
\centering
\caption{Desempenho comparativo (em \%) no conjunto aleatório de 90 instâncias da partição de teste do \textit{media-bias split}.}
\label{tab:resultados_90}
\begin{tabular}{@{}cccc@{}}
\toprule
\textbf{Modelo}                                             & \textbf{\begin{tabular}[c]{@{}c@{}}Configuração\\ (Repre. / Fun. de Perda / Classificador)\end{tabular}} & \textbf{Estratégia} & \textbf{Macro F1-Score} \\ \midrule
Gemini 3                                                    & - / - / Chat Gemini                                                                                       & Zero Shot           & 34.0                    \\
\begin{tabular}[c]{@{}c@{}}Proposto\\ (Exp. 4)\end{tabular} & DistilRoBERTa / Constrastive / KNN                                                                        & Fine-tuning         & 27.0                    \\
DeepSeek                                                    & - / - / Chat DeepSeek                                                                                     & Zero Shot           & 16.0                    \\ \bottomrule
\end{tabular}
\end{table}

Para avaliar a generalização em cenários de estresse ainda mais rigoroso, a Tabela~\ref{tab:resultados_flibias}
apresenta os resultados de transferência de domínio no conjunto FlipBias. Embora as configurações propostas
tenham obtido \textit{Macro F1-scores} de $47.00\%$ (Exp. 16) e $28.21\%$ (Exp. 4)) -- valores inferiores aos modelos
\textit{in-domain}\footnote{In-domain refere-se a uma situação em que os dados utilizados 
para treinar um modelo de aprendizagem de máquina e os dados usados para testá-lo vêm da 
mesma distribuição, contexto ou fonte.} de Lin et al. [2024, 2025] --, os dados evidenciam
a capacidade da arquitetura em capturar estruturas ideológicas fundamentais mesmo sem exposição prévia
ao novo domínio. Adicionalmente, realizou-se uma análise comparativa utilizando uma amostragem aleatória de $90$ 
instâncias do conjunto de teste \textit{media-bias split}. Nesse cenário, o modelo porposto (Exp. 4) alcançou
$27.0\%$ de \textit{Macro F1-score}, superando o desempenho \textit{zero-shot} do \textit{DeepSeek}($16.0\%$) e 
aproximando-se do \textit{Gemini 3} ($34.0\%$), como presentados na Tabela~\ref{tab:resultados_90}. Tais resultados validam a robustez da \textit{Contrastive Loss}, que
demonstra competitividade frente a modelos de linguagem de larga escala em tarefas de inferência direta, mesmo sob 
restrição de dados.
