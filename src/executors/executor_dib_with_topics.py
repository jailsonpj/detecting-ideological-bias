...
from src.preprocess.dataset_custom import NewsPaperData, CustomTopicsDataset
...
class RunDIB:
    ...
    def read_data_topics(self, file_path, to_pytorch=False):
        try:
            # Carrega o array
            data = np.load(file_path)
            
            # Garante que os dados sejam float32 (padrão para redes neurais)
    ...
    def execute(self):   
        ...
        kfold = KFold(
            n_splits=self.config["train"]["folds"], 
            shuffle=True, 
            random_state=42
        )
        ...
        if save_model:
            save_path = os.path.join(
                self.config["path_base_save_model"], 
                self.config["name_process"]
            )
            os.makedirs(save_path, exist_ok=True)
            full_path = os.path.join(
                save_path, 
                f"{self.config['name_process']}_fold{save_fold}_model.pth"
            )
